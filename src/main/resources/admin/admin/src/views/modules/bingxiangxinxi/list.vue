<template>
	<div class="main-content" :style='{"padding":"30px 30px 0"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"width":"100%","margin":"0 0 20px"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"margin":"20px 0","display":"flex"}'>
					...
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"rgba(255, 197, 35, 1)","borderRadius":"40px","background":"rgba(90, 67, 74, 1)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('bingxiangxinxi','冰箱色系')" type="warning" @click="chartDialog3()">冰箱色系</el-button>
				</el-row>
			</el-form>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="zhusexiChart3" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
//统计接口
    chartDialog3() {
      ...
      this.$nextTick(()=>{
        ...
        var zhusexiChart3 = echarts.init(document.getElementById("zhusexiChart3"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                zhusexiChart3.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    zhusexiChart3.resize();
                };
            }
        });
      })
